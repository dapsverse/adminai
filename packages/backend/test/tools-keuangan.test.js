...
describe('create_transaction', () => {
    it('records an income transaction', async () => {
        ...
        const result = await createTransactionTool.execute({ type: 'income', amount: 500000, description: 'Penjualan kopi' }, user.id);
    ...
    it('records an expense transaction with category', async () => {
        const user = await createTestUser();
        const result = await createTransactionTool.execute({ type: 'expense', amount: 150000, category: 'Bahan Baku', description: 'Beli tepung' }, user.id);
        expect(result.success).toBe(true);
        const tx = result.data;
    ...
    it('returns error when date format is invalid', async () => {
        const user = await createTestUser();
        const result = await createTransactionTool.execute({ type: 'income', amount: 100000, date: 'bukan-tanggal' }, user.id);
        expect(result.success).toBe(false);
        expect(result.error).toBeTruthy();
    });
});
describe('get_balance', () => {
    ...
    it('this month summary only includes current month transactions', async () => {
        ...
        await createTransactionTool.execute({ type: 'income', amount: 999999, date: pastDate.toISOString() }, user.id);
        // This month
        await createTransactionTool.execute({ type: 'income', amount: 200000 }, user.id);
        const result = await getBalanceTool.execute({}, user.id);
        const data = result.data;
