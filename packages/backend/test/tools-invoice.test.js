...
describe('create_invoice', () => {
    it('creates an outgoing invoice with auto-generated number', async () => {
        ...
        const result = await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'PT Maju Jaya', items: sampleItems }, user.id);
    ...
    it('calculates totalAmount from items', async () => {
        const user = await createTestUser();
        const result = await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client A', items: sampleItems }, user.id);
        const inv = result.data;
    ...
    it('increments invoice sequence per user per month', async () => {
        const user = await createTestUser();
        const r1 = await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client A', items: [{ description: 'X', qty: 1, price: 100 }] }, user.id);
        const r2 = await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client B', items: [{ description: 'Y', qty: 1, price: 200 }] }, user.id);
        const n1 = r1.data.invoiceNumber;
        const n2 = r2.data.invoiceNumber;
...
describe('list_invoices', () => {
    ...
    it('returns invoices in descending creation order', async () => {
        const user = await createTestUser();
        await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client A', items: [{ description: 'A', qty: 1, price: 100 }] }, user.id);
        await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client B', items: [{ description: 'B', qty: 1, price: 200 }] }, user.id);
        const result = await listInvoicesTool.execute({}, user.id);
    ...
    it('filters by status', async () => {
        const user = await createTestUser();
        await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client A', items: [{ description: 'A', qty: 1, price: 100 }] }, user.id);
        await createInvoiceTool.execute({ direction: 'incoming', clientName: 'Supplier X', items: [{ description: 'B', qty: 1, price: 200 }] }, user.id);
...
describe('mark_invoice_paid', () => {
    it('marks an outgoing invoice as paid and sets paidAt', async () => {
        const user = await createTestUser();
        const created = await createInvoiceTool.execute({ direction: 'outgoing', clientName: 'Client A', items: [{ description: 'A', qty: 1, price: 100 }] }, user.id);
    ...
    it('marks an incoming invoice as paid', async () => {
        const user = await createTestUser();
        const created = await createInvoiceTool.execute({ direction: 'incoming', clientName: 'Supplier X', items: [{ description: 'B', qty: 1, price: 200 }] }, user.id);
        const invoiceId = created.data.id;
        const result = await markInvoicePaidTool.execute({ invoiceId }, user.id);
        expect(result.success).toBe(true);
